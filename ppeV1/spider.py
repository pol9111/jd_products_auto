...
class jdSpider(object):
    ...
    async def adownloader(self, url, loop, key):
        ...
        while has_next:
            ...
            await self.parse(html, redis_conn)

            # 判断是否需要存入mongo
            total_num = await redis_conn.llen('jd_items')
            if total_num >= 3000:
                print('开始存入mongo..')
                await self.save_to_mongo(redis_conn, coll, total_num)
    ...
    async def parse(self, html, redis_conn):
        doc = etree.HTML(html)
        ...
        for item in el:
            ...
            if not exists:
                ...
                print(data)
                await self.save_to_redis(data, redis_conn)


    async def save_to_redis(self, item, redis_conn):
        await redis_conn.lpush('jd_items', str(item))


    async def save_to_mongo(self, redis_conn, coll, total_num):
        items = []
...
def main():
    ...
    db = client['jd']
    bf = redisbloomfilter.BloomFilter(db=0, key='jd_bf', password='qwe123')
